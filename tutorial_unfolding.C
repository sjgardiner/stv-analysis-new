...
void tutorial_unfolding(string inputfile,bool statonly = false) {
  ...
  std::string sysfile = statonly ? "systcalc_statonly.conf" : "systcalc.conf";
  auto* mcc9 = new MCC9SystematicsCalculator(inputfile,sysfile);
  ...
  auto* sb = new SliceBinning( "slice_config.txt" );
  ...
  std::string outfile_name = statonly ? "Plots/StatOnly_UnfoldedData.root" : "Plots/UnfoldedData.root";
  TFile* f_out = TFile::Open(outfile_name.c_str(),"RECREATE");
  ...
  for ( size_t sl_idx = 0u; sl_idx < sb->slices_.size(); ++sl_idx ) {
    ...
    // get some info to label the slice
    bool is_2D = slice.other_vars_.size();

    // assemble a string describing the other variables
    std::string other_var_con = "";
    if(is_2D){
      for(size_t i_ov=0;i_ov<slice.other_vars_.size();i_ov++){ 
        other_var_con = other_var_con + to_string_with_precision(slice.other_vars_.at(i_ov).low_bin_edge_,3) + " < ";
        other_var_con = other_var_con + sb->slice_vars_.at(slice.other_vars_.at(i_ov).var_index_).name_ + " < ";
        other_var_con = other_var_con + to_string_with_precision(slice.other_vars_.at(i_ov).high_bin_edge_,3);
      }
    }
    std::cout << other_var_con << std::endl;

   // std::cout << sb->slice_vars_.size()  << std::endl;
    ...
    for ( const auto& pair : slice_gen_map ) {
      ...
      double min = DBL_MAX;
      for(int i_b=0;i_b<slice_h->hist_->GetNbinsX();i_b++){
        max = std::max(max,slice_h->hist_->GetBinContent(i_b)+slice_h->hist_->GetBinError(i_b));
        min = std::min(min,slice_h->hist_->GetBinContent(i_b)-slice_h->hist_->GetBinError(i_b));
      }
    ...
    slice_unf->hist_->GetYaxis()->SetRangeUser( ymin, ymax*1.2 );
    slice_unf->hist_->Draw( "e same" );
    slice_unf->hist_->SetTitle(other_var_con.c_str());

    TLegend* lg = new TLegend( 0.15, 0.8, 0.8, 0.89 );
    lg->SetNColumns(2);
    for ( const auto& pair : slice_gen_map ) {
      const auto& name = pair.first;
      const auto* slice_h = pair.second;
    ...
    lg->Draw( "same" );
    //string title = static_cast<string>(slice.hist_->GetXaxis()->GetTitle());
    //title.erase(0,1);
    string title = "Slice_" + std::to_string(sl_idx);
    if(statonly) title = "StatOnly_" + title;
    c1->Print(("Plots/"+title+".pdf").c_str());

    slice_unf->hist_->Write(("UnfoldedData_"+title).c_str());
    if(using_fake_data){
      slice_truth->hist_->SetTitle(other_var_con.c_str());
      slice_truth->hist_->Write(("Truth_"+title).c_str());
    }
    slice_cv->hist_->SetTitle(other_var_con.c_str());
    slice_cv->hist_->Write(("CV_"+title).c_str());
  
    // Write the covariance matrix as well
    slice_unf->cmat_.cov_matrix_->SetTitle(other_var_con.c_str());
    slice_unf->cmat_.cov_matrix_->Write(("CovMat_UnfoldedData_"+title).c_str());
