...
void norm() {
  ...
  // Now that we have the accumulated POT over all BNB data runs, sum it into a
  // single number. This will be used to normalize the detVar MC samples.
  ...
  std::map< NFT, std::unique_ptr<TH1D> > data_hists;

  // Holds universe objects for reweightable systematics
  std::map< std::string, std::vector<Universe> > rw_universes;

  // Detector systematic universes (and the detVar CV) will be indexed using
  // ntuple file type values. We're currently scaling one set of ntuples (from
  // Run 3b) to the full dataset.
  // TODO: revisit this procedure if new detVar samples become available
  std::map< NFT, std::unique_ptr<Universe> > detvar_universes;

  TFile in_tfile( "/uboone/data/users/gardiner/respmat-myconfig_delta_pTx.root",
    "read" );

  TDirectoryFile* root_tdir = nullptr;

  in_tfile.GetObject( "mcc9_delta_pTx", root_tdir );
  ...
  for ( const auto& run_and_type_pair : fpm.ntuple_file_map() ) {
    ...
    for ( const auto& type_and_files_pair : type_map ) {
      ...
      for ( const std::string& file_name : file_set ) {
        ...
        std::cout << "PROCESSING response matrices for " << file_name << '\n';
        ...
        // Get the TDirectoryFile name used to store histograms for the current
        // ntuple file
        std::string subdir_name = ntuple_subfolder_from_file_name( file_name );

        TDirectoryFile* subdir = nullptr;
        root_tdir->GetObject( subdir_name.c_str(), subdir );
        if ( !subdir ) {
          throw std::runtime_error( "Missing TDirectoryFile " + subdir_name );
        }

        // For data, just add the reco-space event counts to the total, scaling
        // to the beam-on triggers in the case of EXT data
        if ( !is_mc ) {
          ...
          // If we don't have a histogram in the map for this data type yet,
          // just clone the existing histogram.
          if ( !data_hists.count(type) ) {
            ...
            data_hists[ type ].reset( temp_clone );
          }
          else {
            // Otherwise, just add its contribution to the existing histogram
            data_hists.at( type )->Add( reco_hist );
          }
        ...
        // If we've made it here, then we're working with an MC ntuple file.
        // For these, all three histograms for the "unweighted" universe are
        // always evaluated. Use this to determine the number of true and reco
        // bins easily.
        ...
        if ( is_detVar ) {
          ...
          // Right now, we're assuming there's just one detVar ntuple file per
          // universe. If this assumption is violated, our scaling will be
          // screwed up. Prevent this from happening by throwing an exception
          // when a duplicate is encountered.
          if ( detvar_universes.count(type) ) {
            throw std::runtime_error( "Duplicate detVar ntuple file!" );
          }

          // Otherwise, make a new Universe object to store the (POT-scaled)
          // detVar histograms.
          auto temp_univ_ptr = std::make_unique< Universe >( dv_univ_name,
            0, num_true_bins, num_reco_bins );

          // The detector variation universes are all labeled "unweighted" in
          // the response matrix files. Retrieve the corresponding histograms.
          ...
          // Scale all detVar universe histograms from the simulated POT
          // to the *total* BNB data POT for all runs analyzed. Since we only
          // have detVar samples for Run 3b, we assume that they can be applied
          // globally in this step.
          ...
          // Add the scaled contents of these histograms to the corresponding
          // histograms in the new Universe object
          ...
          detvar_universes[ type ].reset( temp_univ_ptr.release() );
        ...
        // Now handle the reweightable systematic universes
          ...
          // the map of reweighting universes from the 2D histogram keys
          // in its TDirectoryFile.
          // NOTE: I rely here on the reweighting universe definitions being
          // identical across all ntuples considered by the script.
          if ( rw_universes.empty() ) {

            TList* universe_key_list = subdir->GetListOfKeys();
            int num_keys = universe_key_list->GetEntries();

            for ( int k = 0; k < num_keys; ++k ) {
              // To avoid double-counting universes, only create new Universe
              // objects for the 2D event count histograms
              ...
              // We have what we need to create the new Universe object. Do it!
              // Note that its owned histograms are currently empty.
              ...
              if ( !rw_universes.count(univ_name) ) {
                rw_universes[ univ_name ] = std::vector< Universe >();
              }
              ...
              rw_universes.at( univ_name ).emplace_back( std::move(temp_univ) );
          ...
          // For reweightable MC ntuple files, scale the histograms for each
          // universe to the BNB data POT for the current run
          double run_bnb_pot = run_to_bnb_pot_map.at( run );
          double rw_scale_factor = run_bnb_pot / file_pot;

          // Iterate over the reweighting universes, retrieve the histograms
          // for each, and add their POT-scaled contributions from the current
          // ntuple file to the total
          for ( auto& rw_pair : rw_universes ) {
            ...
            for ( size_t u_idx = 0u; u_idx < univ_vec.size(); ++u_idx ) {
              ...
              // Double-check that the universe ordering is right. The index
              // in the map of universes should match the index stored in
              // the Universe object itself. If this check fails, something
              // went wrong with the key sorting imposed by the input
              // TDirectoryFile.
              ...
              subdir->GetObject( (hist_name_prefix + "_reco").c_str(), h_reco );
              subdir->GetObject( (hist_name_prefix + "_true").c_str(), h_true );
  ...
  for ( const auto& pair : data_hists ) {
  ...
  for ( const auto& pair : detvar_universes ) {
    NFT type = pair.first;
  ...
  for ( const auto& pair : rw_universes ) {
