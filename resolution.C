...
//        TCanvas *PlotCanvas(new TCanvas[num_NSamples][1]);
...
//	delete[] PlotCanvas;
	...
	std::string output_file_name = "Plots/resolution/resolution.root";
	TFile *output_file = new TFile( output_file_name.c_str(), "UPDATE" );

	// Open files that contain samples
	for ( int which_sample = 0; which_sample < num_NSamples; which_sample++ ) {
		FileSample[which_sample] = TFile::Open( "myout.root" );
	}
	...
	for ( int which_sample = 0; which_sample < num_NSamples; which_sample++ ) {
		...
		for ( int which_plot = 0; which_plot < num_N2DPlots; which_plot++ ) {
			...
			// Decide what to put into TH1D
                	stv_tree->Draw( ( std::string("mc_") + variable.c_str() + std::string(":") + variable.c_str() + variable_addition.c_str() + std::string(">>h(10,0,") + xmax.c_str() + std::string(",10,0,") + ymax.c_str() + std::string(")") ).c_str(), "passed_ccnp && generated_ccnp", "colz" );
